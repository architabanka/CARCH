...
const uint8_t left_row_shift[16] = {
...
const uint8_t right_row_shift[16] = {
    // shifting the ith row(0 indexed) to left by i in the state matrix (16 bytes filled column wise)
    0, 13, 10,  7,
    4,  1, 14, 11,
    8,  5,  2, 15,
...
std::vector<uint32_t> key_gen(const std::string& password, const std::vector<unsigned char>& salt) {
    ...
    const int iterations = 100000; // Standard security level
    
    out_key.resize(16);

    // PKCS5_PBKDF2_HMAC(pass, pass_len, salt, salt_len, iter, digest, out_len, out_ptr)
    ...
    if (result != 1) {
        std::cerr << "Error generating AES key from password using OpenSSL" << std::endl;
    }
