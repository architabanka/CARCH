#include <openssl/evp.h>
#include <openssl/rand.h>

#include <filesystem>
#include <fstream>
#include <iostream>

#include <cstdint>
#include <vector>
#include <string>
#include <cstring>

const uint8_t sbox[256] = {
    
};

const uint8_t inv_sbox[256] = {
    
};

const uint8_t row_shift[16] = {
    // shifting the ith row(0 indexed) to left by i in the state matrix (16 bytes filled column wise)
    0, 5, 10, 15,
    4, 9, 14, 3,
    8, 13, 2, 7,
    12, 1, 6, 11
};

const uint32_t rcon[10] = {
    0x01000000, 0x02000000, 0x04000000, 0x08000000, 0x10000000, 
    0x20000000, 0x40000000, 0x80000000, 0x1B000000, 0x36000000
};

std::vector<uint32_t> key_gen(const std::string& password) {
    std::vector<uint32_t> key(4, 0);
    return key;
}

std::vector<uint32_t> round_key_gen(const std::vector<uint32_t>& key) {
    std::vector<uint32_t> round_keys(44, 0);

    // Load initial 4 words directly
    for (int i = 0; i < 4; i++) {
        round_keys[i] = key[i];
    }

    for (int i = 4; i < 44; i++) {
        uint32_t temp = round_keys[i - 1];

        if (i % 4 == 0) {
            // 1. RotWord
            temp = (temp << 8) | (temp >> 24);

            // 2. SubWord (extract bytes, apply S-box, reassemble)
            uint8_t b1 = sbox[(temp >> 24) & 0xFF];
            uint8_t b2 = sbox[(temp >> 16) & 0xFF];
            uint8_t b3 = sbox[(temp >> 8) & 0xFF];
            uint8_t b4 = sbox[temp & 0xFF];
            
            temp = (static_cast<uint32_t>(b1) << 24) | 
                   (static_cast<uint32_t>(b2) << 16) | 
                   (static_cast<uint32_t>(b3) << 8)  | 
                    static_cast<uint32_t>(b4);

            temp ^= rcon[(i / 4) - 1];
        }

        round_keys[i] = round_keys[i - 4] ^ temp;
    }
    return round_keys;
}


void encrypt(const std::string& archiveName, const std::string& password){
    std::fstream file(archiveName, std::ios::in | std::ios::out | std::ios::binary | std::ios::ate);
    if (!file) {
        std::cerr << "Archive not Found.";
    }
    // PKCS#7 Method Padding
    // The way we convert the size into multiple of 16 bytes is add 01 as a padding 
    // if we want 1 byte (Note: if it is already multiple of 16 then we pad it with 16 bytes)
    auto padding = (std::filesystem::file_size(archiveName))%16;
    if(padding == 0) {
        padding = 16;
    }
    char myData[padding];
    for(int i = 0; i < padding; i++){
        myData[i] = padding;
    }
    file.write(myData, padding);

    std::vector key = key_gen(password);
    std::vector<uint32_t> round_keys = round_key_gen(key);

    for(int round = 0; round < 10; round++){
        file.seekg(0, std::ios::beg);
        file.clear();
        char buffer[16];

        while (file.read(buffer, 16) || file.gcount() > 0) {
            // Step 1: Byte sub
            for(int i = 0; i < 16; i++){
                buffer[i] = sbox[buffer[i]];
            }
            // Step 2: Row Shifting
            char b[16];
            for(int i = 0; i<16; i++){
                b[i] = buffer[row_shift[i]];
            }
            // Step 3: Mix Columns (Skipped in last round)
            char c[16];
            if(round != 9){
                for(int i = 0; i < 16; i+=4){
                    c[i+0] = 2*b[i] + 3*b[i+1] + 1*b[i+2] + 1*b[i+3];
                    c[i+1] = 1*b[i] + 2*b[i+1] + 3*b[i+2] + 1*b[i+3]; 
                    c[i+2] = 1*b[i] + 1*b[i+1] + 2*b[i+2] + 3*b[i+3]; 
                    c[i+3] = 3*b[i] + 1*b[i+1] + 1*b[i+2] + 2*b[i+3]; 
                }
            }
            else{
                for(int i = 0; i < 16; i++){
                    c[i] = b[i];
                }
            }
            // Step 4: Add Round key (from (round + 1)* to (round+1)*4 + 3)
            int base_idx = (round+1)*4;

            for (int i = 0; i < 4; i++) {
                uint32_t buffer_word = 
                    (static_cast<uint8_t>(buffer[i * 4]) << 24)     |
                    (static_cast<uint8_t>(buffer[i * 4 + 1]) << 16) |
                    (static_cast<uint8_t>(buffer[i * 4 + 2]) << 8)  |
                    (static_cast<uint8_t>(buffer[i * 4 + 3]));

                buffer_word ^= round_keys[base_idx + i];

                buffer[i * 4]     = static_cast<char>((buffer_word >> 24) & 0xFF);
                buffer[i * 4 + 1] = static_cast<char>((buffer_word >> 16) & 0xFF);
                buffer[i * 4 + 2] = static_cast<char>((buffer_word >> 8)  & 0xFF);
                buffer[i * 4 + 3] = static_cast<char>(buffer_word & 0xFF);
            }
            // rewrite that to archive
            file.seekp(-16, std::ios::cur);
            file.write(buffer, 16);
        }
    }
}
void decrypt(const std::string& sourcePath, const std::string& destPath, std::string password){
    // we check if there is the magic sting in the decrypted file (that is our password chcker basically)
};